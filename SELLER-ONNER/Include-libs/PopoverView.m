...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withText:(NSString *)text
{
    ...
    CGRect textSize = [text boundingRectWithSize:CGSizeMake(screenSize.width - kHorizontalMargin*4.f, 1000.f)
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withViewArray:(NSArray *)viewArray
{
    ...
    for (UIView *view in viewArray) {
        ...
        view.frame = CGRectMake(0, (viewArray.count>1)? totalHeight : (container.bounds.size.height/2)-kBoxPadding/2, view.frame.size.width, view.frame.size.height);
    ...
    float padding = (viewArray.count != 1) ? 0.f : kBoxPadding;
    
    container.frame = CGRectMake(0, 0, totalWidth, totalHeight - (padding*2));
...
-(void)setupLayout:(CGPoint)point inView:(UIView*)view
{
    ...
    //DLog(@"arrowPoint:%f,%f", arrowPoint.x, arrowPoint.y);

    CGRect topViewBounds = topView.bounds;
    //DLog(@"topViewBounds %@", NSStringFromCGRect(topViewBounds));
    ...
    if (arrowPoint.x + kArrowHeight > topViewBounds.size.width - kHorizontalMargin - kBoxRadius - kArrowHorizontalPadding) {//Too far to the right
        ...
        //DLog(@"Correcting Arrow Point because it's too far to the right");
        ...
        //DLog(@"Correcting Arrow Point because it's too far to the left");
    }
    ...
    //DLog(@"boxFrame:(%f,%f,%f,%f)", boxFrame.origin.x, boxFrame.origin.y, boxFrame.size.width, boxFrame.size.height);
...
{    
    ...
    //DLog(@"subviewsArray:%@", subviewsArray);
    
    for (int i = 0; i < subviewsArray.count && !found; i++) {
        UIView *view = [subviewsArray objectAtIndex:i];
        
        //DLog(@"Rect:(%f,%f,%f,%f)", view.frame.origin.x, view.frame.origin.y, view.frame.size.width, view.frame.size.height);
    ...
    if (!found && CGRectContainsPoint(contentView.bounds, point)) {
        found = YES;
        //DLog(@"popover box contains point, ignoring user input");
    }
